    ...
    <div style={{ maxWidth: "100%" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {results.articles.map(({ num, title, text }) => (
              <button key={num} onClick={() => onArticleClick(num)}
                ...
                <p style={{ color: "#94a3b8", fontSize: 13, margin: "6px 0 0", lineHeight: 1.5 }}>{renderHighlightedParts(highlightText(truncateText(text, 200), query))}</p>
              </button>
            ))}
          </div>
        </div>
      ...
      {results.articles.length === 0 && results.recitals.length === 0 && (
        <div style={{ textAlign: "center", padding: "64px 0" }}>
          ...
          <p style={{ color: "#94a3b8", fontSize: 16, fontFamily: SANS }}>No results found for "{query}"</p>
